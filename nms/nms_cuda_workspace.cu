  ...
  const int64_t num_boxes)
{
  ...
  if(idx >= midpt)
  {
    box_x_idx = num_boxes - base - 2;
    box_y_idx = idx - midpt + box_x_idx + 1;
    box_x_idx = sorted_idx[box_x_idx+num_boxes * blockIdx.y] + num_boxes * blockIdx.y;
    box_y_idx = sorted_idx[box_y_idx+num_boxes * blockIdx.y] + num_boxes * blockIdx.y;
  }
  else
  {
    box_x_idx = base;
    box_y_idx = idx - midpt + num_boxes;
    box_x_idx = sorted_idx[box_x_idx+num_boxes*blockIdx.y] + num_boxes * blockIdx.y;
    box_y_idx = sorted_idx[box_y_idx+num_boxes*blockIdx.y] + num_boxes * blockIdx.y;
  }
...
std::tuple<Tensor, Tensor> non_max_suppression_cuda(const Tensor& input, const Tensor& scores, const double thresh)
{
  ...
  unsigned char *elims;
  cudaMalloc(&elims, n_pairs*batch_size*sizeof(unsigned char));
