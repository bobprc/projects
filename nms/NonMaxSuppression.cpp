  ...
  for(int batch=0; batch<batch_size; ++batch)
  {
    ...
    while(pos < (1+batch)*num_boxes-1)
    {
      for(int i=pos+1; i<num_boxes*(1+batch); ++i)
      {
        ...
        int idx_y = rawIdx[i];
        float lr = std::fmin(rawInput[idx_x] + rawInput[idx_x+2],
                             rawInput[idx_y] + rawInput[idx_y+2]);
        float rl = std::fmax(rawInput[idx_x], rawInput[idx_y]);
        float tb = std::fmin(rawInput[idx_x+1] + rawInput[idx_x+3],
                             rawInput[idx_y+1] + rawInput[idx_y+3]);
        float bt = std::fmax(rawInput[idx_x+1], rawInput[idx_y+1]);
        float inter = std::fmax(0, lr-rl)*std::fmax(0, tb-bt);
        float uni = (rawInput[idx_x+2]*rawInput[idx_x+3] 
                     + rawInput[idx_y+2]*rawInput[idx_y+3] - inter);
        if (inter/uni > thresh)
          rawMask[i] = 0;
      }
      ++pos;
